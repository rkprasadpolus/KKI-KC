	...
	public static boolean isUserRouteLogAuthenticated(String documentId) {
		...
		if(userSession!=null){
			...
			try {
				...
				LOG.debug("Invalid format documentId (should be LONG): " + documentId, e);
		    } catch (RiceRuntimeException e) {
		    	LOG.error("Runtime Exception checking if user is route log authenticated: userId: " + principalId + ";documentId: " + documentId, e);
