		...
		if ( StringUtils.isNotBlank(attrib.getKimAttribute().getComponentName()) ) {
			try {
				...
				try {
					...
					LOG.error( "Got a ClassNotFoundException resolving a values finder - since this should have been executing in the context of the host system - this should not happen.", ex);
					return Collections.emptyList();
				}
			} catch ( ClassNotFoundException ex ) {
				LOG.error( "Got a ClassNotFoundException resolving a component name (" + attrib.getKimAttribute().getComponentName() + ") - since this should have been executing in the context of the host system - this should not happen.", ex);
			}
		} else {
